...
export default function CollectionDetail() {
  ...
  const editBorder  = useThemeColor({ light: "#e0e0e0",   dark: "#3a3a3c" }, "border");
  const verseText   = useThemeColor({ light: "#444444",   dark: "#ebebf5" }, "text");
  const removeTint  = useThemeColor({ light: "#cccccc",   dark: "#48484a" }, "tabIconDefault");
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: bg }]} edges={["top"]}>
      ...
      <View style={[styles.titleRow, { borderBottomColor: border }]}>
        <Text style={[styles.pageTitle, { color: textPrimary }]}>{collection.name}</Text>
        {collection.description ? (
          <Text style={[styles.description, { color: textMuted }]}>{collection.description}</Text>
        ) : null}
            ...
            return (
              ...
              >
                <View style={styles.cardHeader}>
                  <Text style={[styles.ref, { color: textPrimary }]}>{ref}</Text>
                  <View style={styles.cardHeaderRight}>
                    {item.isMemorized && (
                      <Text style={[styles.memorizedDot, { color: memorizedCheck }]}>✓</Text>
                    )}
                    {canRemove && (
                      <TouchableOpacity
                      ...
                      >
                        <Text style={[styles.removeText, { color: removeTint }]}>✕</Text>
                      </TouchableOpacity>
                    )}
                  </View>
                </View>
                <Text style={[styles.verseText, { color: verseText }]} numberOfLines={3}>
                  {item.text}
                </Text>
              </TouchableOpacity>
            );
...
const styles = StyleSheet.create({
  ...
  pageTitle: {
    fontSize: 26, fontWeight: "700", letterSpacing: -0.6, marginBottom: 2,
  },
